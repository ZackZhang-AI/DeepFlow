...
export function Timeline({ events }: Props) {
  ...
  return (
    <div className="bg-slate-800/20 border border-slate-800 rounded-xl p-4">
      <h3 className="text-xs font-semibold text-slate-500 mb-3 uppercase tracking-wider">执行时间线</h3>
      <div className="space-y-2">
        {events.map((event, i) => {
          const info = EVENT_LABELS[event.type] || { label: event.type, color: "text-slate-500" };
          const elapsed = i === 0 ? "0s" : `${((event.time - events[0].time) / 1000).toFixed(0)}s`;
          ...
          return (
            <div key={i} className="flex items-start gap-3 text-xs">
              <div className="flex flex-col items-center pt-0.5">
                ...
                />
                {i < events.length - 1 && <div className="w-px h-full min-h-[16px] bg-slate-700 mt-0.5" />}
              </div>
              ...
              <div className="flex-1 min-w-0 pb-2">
                ...
                <div className="text-slate-500 mt-0.5 truncate">
                  {event.type === "step.started" &&
                    `步骤 ${event.data.step_index}/${event.data.total_steps}: ${event.data.title}`}
                  {event.type === "step.completed" &&
                    `步骤 ${event.data.step_index} 完成（${event.data.sources_count} 个来源）`}
                  {event.type === "planner.completed" && `${event.data.steps_count} 个研究步骤`}
                  {event.type === "report.completed" &&
                    `${event.data.sources_count} 个来源，${event.data.tokens_used} tokens`}
