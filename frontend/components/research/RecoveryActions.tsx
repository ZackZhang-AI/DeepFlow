...
export function RecoveryActions({ task, busy, onReconnect, onRetry }: RecoveryActionsProps) {
  return (
    <section className="rounded-xl border border-red-200 bg-red-50 p-5 text-center" aria-labelledby="recovery-heading">
      ...
      <h2 id="recovery-heading" className="mt-4 text-xl font-semibold text-slate-950">研究未能继续</h2>
      ...
      <div className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
        ...
        <Link href="/" className="inline-flex min-h-11 items-center justify-center rounded-xl px-4 text-sm font-medium text-slate-700 hover:bg-white">返回新研究</Link>
      </div>
    </section>
  );
}
