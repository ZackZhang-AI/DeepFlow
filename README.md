...
- 用户注册、登录、会话鉴权。
- 创建研究任务，支持最多三轮渐进澄清、研究计划生成、自然语言修改、计划确认和可选自动确认 API。
...
- 完成态区分完整覆盖与部分覆盖，并展示每个研究问题的来源数量。
- 标准/深度档在规划前执行一次可降级背景搜索；快速档不增加规划搜索调用。
...
- 来源检查展示来源对应的研究步骤和引用所在论述，便于从结论回查证据。
...
- Markdown 报告生成、查看和编辑。
- 报告完成前执行结构与引用质量检查，并支持基于当前报告和已记录来源追问。
