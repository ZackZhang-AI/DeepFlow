...
- 只分析用户当前任务和提供的前序研究发现，不要引入无关业务场景或自造主题
- 保留前序研究中的实体、趋势、产品或指标原名，不要随意重命名为 Agent A/B、方案A/B 等泛化名称
- 如果前序研究缺少可计算数据，用小型、明确标注的示例数据演示方法，并清楚说明限制
...
- 不要保存图片或任何文件；如需可视化，只输出表格数据或 ASCII 摘要
...
- 不要把任务替换成 ROI、财务、营销等未被用户要求的分析
- 不要把“趋势A/趋势B”误读为“Agent A/Agent B”
